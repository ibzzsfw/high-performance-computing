...
int main(int argc, char **argv) {
  ...
  if (rank == TOP_RANK) {
    ...
    if (size > 1) {
      // using non-blocking MPI

      // send amount to other processes
      ...
      // receive matrix C from other processes non-blocking
      for (int rank = 1; rank < size; rank++) {
        int start_index = 0;
        int end_index = 0;
        ...
        // receive matrix C for non-blocking
        MPI_Irecv(&matrixC[start_index], amount, MPI_DOUBLE, rank, 3, MPI_COMM_WORLD, &request[0]);

        // wait for matrix C to be received
    ...
    // receive amount non-blocking
    MPI_Irecv(&amount, 1, MPI_INT, TOP_RANK, 0, MPI_COMM_WORLD, &request[1]);

    // wait for amount to be received
    ...
    // non-blocking receive matrix A
    MPI_Irecv(matrixA, amount, MPI_DOUBLE, TOP_RANK, 1, MPI_COMM_WORLD, &request[2]);

    // non-blocking receive matrix B
    MPI_Irecv(matrixB, amount, MPI_DOUBLE, TOP_RANK, 2, MPI_COMM_WORLD, &request[3]);

    // wait for matrix A and B to be received
